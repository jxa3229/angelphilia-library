...
本功能所有 UI、JSON、spec 和复制内容均不写具体网站、平台、链接、作者或截图来源。对用户可见的来源统一显示为“网络数据”。内部仅保留泛化 `sourceId` 供校验引用：
...
|-----------|------|------|------|--------|----------|
| C1 | 16.8cm | 13.1cm | 21.5cm | 13.6cm | 完整围度 |
...
`frame`、小腿、上臂、下臂、手、脚等默认件使用 `Obitsu default` 作为部件体系语义。它不是来源展示名；来源仍统一为“网络数据”。
